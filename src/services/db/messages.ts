...
export async function upsertMessage(message: Message): Promise<void> {
  const db = await getDb();
  await db.execute(
    `INSERT INTO messages (id, thread_id, account_id, from_address, from_name, to_addresses, cc_addresses, bcc_addresses, subject, snippet, body_html, body_text, date, is_read, has_attachments, header_message_id)
